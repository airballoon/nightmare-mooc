...
var cityList = _.keys(city);

firstUrl_list = _.map(cityList, function(code) {
  return 'http://' + code + 'speiyou.com/search/index/subject:/grade:0/gtype:time';
});

lastUrl_list = _.map(cityList, function(code) {
  return 'http://' + cityCode + 'speiyou.com/search/index/grade:0/level:bx/subject:/gtype:time/service:/time:/term:/period:/o:da/bg:n/curpage:';
};
...
var fetch = exports.fetch = function() {
  ...
  _.each(firstUrl_list, function(url, index) {
    nightmare
      .goto(url)
    	.evaluate(function () {
        ...
        if (pageSum > 1) {
          ...
          .evaluate(function () {
            ...
            console.log(path);
            var buffer = new Buffer(city[index] + sum + '\n');
